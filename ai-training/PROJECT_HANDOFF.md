...
目前 working tree 有 `ai-training/` 與 `package.json` 相關改動。

建議先在目前工作建立分支：

```bash
git checkout -b ai-training-data
git add package.json ai-training
git commit -m "Add AI training data pipeline"
```

另一個 Codex session 若要做主程式功能開發，建議另開分支，避免同時改 `src/main.js`、`src/aiParser.js`、`package.json`。
